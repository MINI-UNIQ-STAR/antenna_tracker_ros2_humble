  ...
  double & az_output, double & el_output)
{
  ...
  yref[3] = 0.0;
  yref[4] = 0.0;                              // u_az ref
  yref[5] = kGravityCoeff * std::cos(el_target_rad);  // u_el ref = gravity equilibrium at target
  ...
  for (int i = 0; i <= ANTENNA_TRACKER_N; ++i) {
    ...
    if (i < ANTENNA_TRACKER_N) {
      ...
      double u_ws[ANTENNA_TRACKER_NU] = {
        0.0,
        kGravityCoeff * std::cos(el_interp)  /* gravity equilibrium along interpolated path */
      };
